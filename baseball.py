...
def filter_by_year(statistics, year, yearid):
    ...
    """
    filter_by_year = list(filter(lambda pair: pair[yearid] == str(year), statistics))
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for dic in statistics:
        #print(dic)
        #formula(info,dic)
        result[dic[playerid]]=formula(info,dic)
    newlist=[(k,v) for v,k in sorted([(v,k) for k,v in result.items()],reverse=True)]
    finallist=newlist[:numplayers]
    return finallist
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    master=read_csv_as_nested_dict(info['masterfile'], info['playerid'],info['separator'],info['quote'])
    result=[]
    for tup in top_ids_and_stats:
        record=master[tup[0]]
        string='{:.3f}'.format(tup[1])+' --- '+record[info['firstname']]+' '+record[info['lastname']]
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    top_player=top_player_ids(info, statistics_by_year, formula, numplayers)
    result=lookup_player_names(info, top_player)
    return result
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for record in statistics:
        ...
        if key not in result:
            result[key]={}
            result_agg=result[key]    
            result_agg[playerid]=key
            for field in fields:
                if record[field]!='NA':
                    result_agg[field]=int(record[field])
        else:
            result_agg=result[key]    
            for field in fields:
                if record[field]!='NA':
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    agg=aggregate_by_player_id(statistics, info['playerid'], fields)
    agg1=list(agg.values())
    top_player=top_player_ids(info,agg1, formula, numplayers)
    result=lookup_player_names(info, top_player)
    return result
